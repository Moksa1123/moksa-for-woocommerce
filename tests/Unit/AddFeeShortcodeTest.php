...
final class AddFeeShortcodeTest extends TestCase {
	...
	private static function eval_for_temps( array $temps, array $extra = [] ): float {
		return self::eval_atts( array_merge( [ 'moksafowo-temps' => implode( ',', $temps ) ], $extra ) );
	}
	...
	public function test_qty_multiplies_by_cart_quantity(): void {
		self::assertSame( 50.0, self::eval_atts( [ 'qty' => '10', 'moksafowo-qty' => '5' ] ) );
		// qty 屬性 0 → 不加
		self::assertSame( 0.0, self::eval_atts( [ 'qty' => '0', 'moksafowo-qty' => '5' ] ) );
		// cart qty 0 → 不加
		self::assertSame( 0.0, self::eval_atts( [ 'qty' => '10', 'moksafowo-qty' => '0' ] ) );
	}

	public function test_weight_multiplies_by_cart_weight(): void {
		self::assertSame( 60.0, self::eval_atts( [ 'weight' => '20', 'moksafowo-weight' => '3' ] ) );
		// 小數重量：20 × 1.5 = 30
		self::assertSame( 30.0, self::eval_atts( [ 'weight' => '20', 'moksafowo-weight' => '1.5' ] ) );
	}

	public function test_combined_temp_n_plus_weight(): void {
		// per-temp base + 每公斤
		$atts = [ 'temp_3' => '200', 'weight' => '20', 'moksafowo-weight' => '2' ];
	...
	public function test_legacy_pattern_base_plus_cool_2_cool_3_still_works(): void {
		// 模擬 `100 + [moksafowo_addfee cool_2="50" cool_3="100"]` 的 shortcode 部分
	...
	public function test_no_moksafowo_temps_attribute_treats_as_no_temps(): void {
		// moksafowo-temps 未注入 → temps array 是空 → 所有 temp_N / cool* 都不 fire
		self::assertSame( 0.0, self::eval_atts( [ 'temp_1' => '100', 'cool' => '60', 'cool_3' => '100' ] ) );
	}

	public function test_invalid_moksafowo_temps_string_filtered_out(): void {
		// 含非數字 token → array_filter(intval) 過濾掉 0 → 剩 1
		self::assertSame( 100.0, self::eval_atts( [ 'moksafowo-temps' => 'foo,1,bar,', 'temp_1' => '100', 'temp_2' => '150' ] ) );
	}
}
