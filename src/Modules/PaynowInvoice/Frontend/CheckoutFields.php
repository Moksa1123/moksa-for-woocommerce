...
final class CheckoutFields {
	...
	public static function init(): void {
		...
		if ( 'yes' === get_option( 'moksafowo_ezpay_invoice_enabled', 'no' ) ) {
			return;
		}
		if ( 'yes' === get_option( 'moksafowo_smilepay_invoice_enabled', 'no' ) ) {
			return;
		}
	...
	public static function render_classic(): void {
		$prefix        = 'moksafowo_paynow_invoice';
		...
		echo '<div class="moksafowo-paynow-invoice-fields"><h3>' . esc_html__( '電子發票', 'mo-ectools' ) . '</h3>';
	...
	private static function register_block_fields(): void {
		...
		// PayNow 不支援會員載具，只有手機條碼 / 自然人憑證 / 紙本
		$carrier_labels  = [
			'mobile' => __( '手機條碼', 'mo-ectools' ),
			'cert'   => __( '自然人憑證', 'mo-ectools' ),
			'paper'  => __( '紙本發票', 'mo-ectools' ),
