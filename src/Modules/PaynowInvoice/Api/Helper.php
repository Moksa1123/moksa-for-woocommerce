...
final class Helper extends AbstractCredentialHelper {
	...
	public static function mem_cid(): string {
		$key = self::is_sandbox() ? 'mo_paynow_invoice_sandbox_mem_cid' : 'mo_paynow_invoice_mem_cid';
		return (string) get_option( $key, '' );
	}

	public static function mem_password(): string {
		$key = self::is_sandbox() ? 'mo_paynow_invoice_sandbox_mem_password' : 'mo_paynow_invoice_mem_password';
		return (string) get_option( $key, '' );
	}
	...
	public static function generate_orderno( int $order_id ): string {
		$prefix = (string) get_option( 'mo_paynow_invoice_orderno_prefix', '' );
