...
final class CardRenderers {
	...
	private static function render_payuni( \WC_Order $order ): string {
		...
		if ( '' === $mer_trade_no && '' === $trade_no ) {
			ob_start();
			echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $method_title ?: 'PAYUNi' ) . '</p>';
			echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'Not paid yet — waiting for the customer to finish the PAYUNi payment flow.', 'moksa-for-woocommerce' ) . '</p>';
		...
		echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $method_title ?: 'PAYUNi' ) . '</p>';
		if ( '' !== $trade_no ) {
			echo '<p><strong>' . esc_html__( 'PAYUNi transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $trade_no ) . '</p>';
		}
		if ( '' !== $mer_trade_no ) {
			echo '<p><strong>' . esc_html__( 'Merchant transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $mer_trade_no ) . '</p>';
		}
		if ( '' !== $card4no ) {
			echo '<p><strong>' . esc_html__( 'Last four card digits:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $card4no );
			if ( '' !== $bank ) {
		...
		if ( '' !== $auth_code ) {
			echo '<p><strong>' . esc_html__( 'Authorization code:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $auth_code ) . '</p>';
		}
		if ( '' !== $inst && '0' !== $inst ) {
			echo '<p><strong>' . esc_html__( 'Installments:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $inst ) . esc_html__( ' payments', 'moksa-for-woocommerce' ) . '</p>';
		}
		if ( '' !== $paid_at ) {
			echo '<p><strong>' . esc_html__( 'Paid at:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $paid_at ) . '</p>';
		}
		return (string) ob_get_clean();
	}
	...
	private static function render_newebpay( \WC_Order $order ): string {
		...
		if ( '' === $mtn && '' === $trade_no ) {
			ob_start();
			echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $method_title ?: 'NewebPay' ) . '</p>';
			echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'Not paid yet — waiting for the customer to complete payment on the NewebPay page.', 'moksa-for-woocommerce' ) . '</p>';
			return (string) ob_get_clean();
		}

		ob_start();
		echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::newebpay_pay_type_label( $pay_type ) ?: ( $method_title ?: 'NewebPay' ) ) . '</p>';
		if ( '' !== $trade_no ) {
			echo '<p><strong>' . esc_html__( 'NewebPay transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $trade_no ) . '</p>';
		}
		if ( '' !== $mtn ) {
			echo '<p><strong>' . esc_html__( 'Merchant transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $mtn ) . '</p>';
		}
		if ( '' !== $card4no ) {
			echo '<p><strong>' . esc_html__( 'Last four card digits:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $card4no ) . '</p>';
		}
		if ( '' !== $pay_time ) {
			echo '<p><strong>' . esc_html__( 'Paid at:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $pay_time ) . '</p>';
		}
		...
		if ( 'CVS' === $pay_type ) {
			...
			if ( '' !== $code ) {
				echo '<p><strong>' . esc_html__( 'Payment code:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $code ) . '</span></p>';
			}
			if ( '' !== $expire ) {
				echo '<p><strong>' . esc_html__( 'Pay before:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $expire ) . '</p>';
			}
			...
			if ( '' !== $acct ) {
				echo '<p><strong>' . esc_html__( 'Virtual account:', 'moksa-for-woocommerce' ) . '</strong>';
				if ( '' !== $bank ) {
					echo esc_html__( 'Bank ', 'moksa-for-woocommerce' ) . esc_html( $bank ) . ' - ';
				}
			...
			for ( $i = 1; $i <= 3; $i++ ) {
				...
				if ( '' !== $bc ) {
					...
					echo '<p><strong>' . esc_html( sprintf( __( 'Barcode %d:', 'moksa-for-woocommerce' ), $i ) ) . '</strong><span style="font-family:monospace;">' . esc_html( $bc ) . '</span></p>';
				}
			}
		}
	...
	private static function render_linepay( \WC_Order $order ): string {
		...
		if ( '' === $tx && '' === $status ) {
			ob_start();
			echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html__( 'LINE Pay', 'moksa-for-woocommerce' ) . '</p>';
			echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'Not paid yet — waiting for the customer to complete payment in LINE Pay.', 'moksa-for-woocommerce' ) . '</p>';
			return (string) ob_get_clean();
		}
		...
		echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html__( 'LINE Pay', 'moksa-for-woocommerce' ) . '</p>';
		if ( '' !== $tx ) {
			echo '<p><strong>' . esc_html__( 'LINE Pay transaction ID:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $tx ) . '</span></p>';
		}
		if ( '' !== $order_id ) {
			echo '<p><strong>' . esc_html__( 'Merchant transaction ID:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $order_id ) . '</span></p>';
		}
		if ( '' !== $status_label ) {
			echo '<p><strong>' . esc_html__( 'Transaction status:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $status_label ) . '</p>';
		}
		if ( '' !== $pay_type && 'NORMAL' !== $pay_type ) {
			echo '<p><strong>' . esc_html__( 'Payment type:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $pay_type ) . '</p>';
		}
		if ( '' !== $auth_amt ) {
			echo '<p><strong>' . esc_html__( 'Authorized amount:', 'moksa-for-woocommerce' ) . '</strong>NT$' . esc_html( (string) (int) $auth_amt ) . '</p>';
		}
		if ( '' !== $auth_at ) {
			echo '<p><strong>' . esc_html__( 'Authorized at:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $auth_at ) . '</p>';
		}
		if ( '' !== $refund_tx ) {
			echo '<p style="color:#646970;font-size:11px;"><strong>' . esc_html__( 'Refund transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $refund_tx ) . '</p>';
		}
		return (string) ob_get_clean();
	}
	...
	private static function render_ezpay_invoice( \WC_Order $order ): string {
		...
		if ( '' !== $inv ) {
			if ( 'zero' === $inv ) {
				echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'The order total is 0, so no invoice was issued.', 'moksa-for-woocommerce' ) . '</p>';
			} elseif ( 'negative' === $inv ) {
				echo '<p style="color:#d63638;font-size:12px;">' . esc_html__( 'The order total is negative, so an invoice cannot be issued.', 'moksa-for-woocommerce' ) . '</p>';
			} else {
				...
				echo '<p><strong>' . esc_html__( 'Invoice service:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html__( 'ezPay', 'moksa-for-woocommerce' ) . '</p>';
				if ( '' !== $type ) {
					echo '<p><strong>' . esc_html__( 'Invoice type:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::invoice_type_label( $type ) ) . '</p>';
				}
				if ( '' !== $ubn ) {
				...
				if ( '' !== $invalid_at ) {
					echo '<p style="color:#d63638;"><strong>' . esc_html__( 'Voided:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $invalid_at ) . '</p>';
				}
			}
		} else {
			// 還沒開立發票，顯示已選的類型 + 「待開立」
			...
			if ( '' !== $carrier_t && 'b2b' !== $type ) {
				echo '<p><strong>' . esc_html__( 'Carrier:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::ezpay_carrier_label( $carrier_t ) ) . '</p>';
			}
			echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'Not issued yet — you can issue the invoice manually here.', 'moksa-for-woocommerce' ) . '</p>';
		}
	...
	private static function invoice_type_label( string $type ): string {
		return [
			'b2c_carrier' => __( 'Individual (with carrier)', 'moksa-for-woocommerce' ),
			'b2b'         => __( 'Company (triplicate)', 'moksa-for-woocommerce' ),
	...
	private static function ezpay_carrier_label( string $carrier ): string {
		return [
			'member' => __( 'ezPay member carrier', 'moksa-for-woocommerce' ),
	...
	private static function render_smilepay_invoice( \WC_Order $order ): string {
		...
		if ( '' !== $inv ) {
			if ( 'zero' === $inv ) {
				...
				echo '<p><strong>' . esc_html__( 'Invoice service:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html__( 'SmilePay', 'moksa-for-woocommerce' ) . '</p>';
				if ( '' !== $type ) {
					echo '<p><strong>' . esc_html__( 'Invoice type:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::invoice_type_label( $type ) ) . '</p>';
				}
				if ( '' !== $ubn ) {
			...
			echo '<p><strong>' . esc_html__( 'Invoice type:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::invoice_type_label( $type ) ) . '</p>';
			if ( '' !== $carrier_t && 'b2b' !== $type ) {
				echo '<p><strong>' . esc_html__( 'Carrier:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::smilepay_carrier_label( $carrier_t ) ) . '</p>';
			}
			echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'Not issued yet — you can issue the invoice manually here.', 'moksa-for-woocommerce' ) . '</p>';
		}
	...
	private static function render_refund_block( \WC_Order $order ): string {
		...
		echo '<p style="margin:0 0 4px;color:#646970;font-size:11px;text-transform:uppercase;letter-spacing:.4px;">' . esc_html__( 'Refund history', 'moksa-for-woocommerce' ) . '</p>';
		foreach ( $refunds as $refund ) {
			if ( ! $refund instanceof \WC_Order_Refund ) {
				continue;
			}
		...
		echo '<strong>' . esc_html__( 'Net order total:', 'moksa-for-woocommerce' ) . '</strong>';
		echo '<span style="color:' . esc_attr( $net_color ) . ';font-weight:600;">NT$' . esc_html( (string) (int) $net ) . '</span>';
		if ( $is_full ) {
			echo ' <span style="color:#d63638;font-size:11px;">(' . esc_html__( 'Full refund', 'moksa-for-woocommerce' ) . ')</span>';
		}
		echo '</p>';
		echo '</div>';
		return (string) ob_get_clean();
	...
	private static function linepay_status_label( string $raw ): string {
		return [
			'reserved'  => __( 'Pre-authorized (awaiting capture)', 'moksa-for-woocommerce' ),
			'authed'    => __( 'Authorized (awaiting capture)', 'moksa-for-woocommerce' ),
			'confirmed' => __( 'Captured', 'moksa-for-woocommerce' ),
			'cancelled' => __( 'Cancelled', 'moksa-for-woocommerce' ),
