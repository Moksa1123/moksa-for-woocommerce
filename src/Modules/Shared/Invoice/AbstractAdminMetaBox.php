...
abstract class AbstractAdminMetaBox {
	...
	public static function add_card( array $cards, \WC_Order $order ): array {
		...
		if ( '' !== $inv && 'zero' !== $inv && 'negative' !== $inv ) {
			...
			echo '<p><strong>' . esc_html__( '發票開立：', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::issue_label( $type ) ) . '</p>';
			if ( '' !== $ubn ) {
			...
			if ( '' !== $carrier_t && 'b2b' !== $type ) {
				echo '<p><strong>' . esc_html__( '載具類型：', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::carrier_label( $carrier_t ) ) . '</p>';
			}
			if ( '' !== $carrier_n ) {
			...
			if ( '' !== $invalid_at ) {
				...
				echo '<button type="button" class="button moksafowo-invoice-invalid">' . esc_html__( '作廢發票', 'moksa-for-woocommerce' ) . '</button> ';
				if ( static::supports_allowance() ) {
					echo '<button type="button" class="button moksafowo-invoice-allowance">' . esc_html__( '開立折讓單', 'moksa-for-woocommerce' ) . '</button>';
				}
				echo '</p>';
				...
				echo '<button type="button" class="button button-primary moksafowo-invoice-invalid-confirm">' . esc_html__( '確認作廢', 'moksa-for-woocommerce' ) . '</button> ';
				...
				if ( static::supports_allowance() ) {
					...
					echo '<button type="button" class="button button-primary moksafowo-invoice-allowance-confirm">' . esc_html__( '確認折讓', 'moksa-for-woocommerce' ) . '</button> ';
			...
			if ( '' !== $allowance_no ) {
				...
				echo '<p><strong>' . esc_html__( '折讓單號：', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $allowance_no );
				if ( '' !== $amt ) {
					echo ' (' . esc_html__( '金額：', 'moksa-for-woocommerce' ) . esc_html( $amt ) . ')';
				}
				echo '</p>';
			}
		} elseif ( 'zero' === $inv || 'negative' === $inv ) {
			echo '<p style="color:#646970;">' . esc_html__( '訂單金額為 0 或負，未開立發票。', 'moksa-for-woocommerce' ) . '</p>';
		} else {
		...
		$cards[] = [
			'slot'  => 'invoice',
			'title' => sprintf( /* translators: %s: provider label */ __( '%s 發票資訊', 'moksa-for-woocommerce' ), static::provider_label() ),
	...
	protected static function render_issue_controls( \WC_Order $order ): void {
		...
		echo '<button type="button" class="button moksafowo-invoice-update">' . esc_html__( '更新', 'moksa-for-woocommerce' ) . '</button> ';
		echo '<button type="button" class="button button-primary moksafowo-invoice-issue" disabled>' . esc_html__( '開立發票', 'moksa-for-woocommerce' ) . '</button>';
		echo '</p>';
	}
	...
	public static function ajax_allowance(): void {
		...
		if ( null === $callable ) {
			wp_send_json_error( [ 'message' => __( '此 provider 不支援折讓。', 'moksa-for-woocommerce' ) ] );
		}
