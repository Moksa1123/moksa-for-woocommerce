...
namespace Moksafowo\Modules\SmilepayInvoice\Api;
	...
	public static function post( string $path, array $args ): array {
		...
		if ( 200 !== $code ) {
			return [
				...
				'message' => sprintf( __( 'SmilePay returned HTTP %d', 'moksa-for-woocommerce' ), $code ),
			];
		}
		...
		if ( ! is_object( $xml ) ) {
			return [
				'ok'      => false,
				'status'  => 'PARSE_FAIL',
				'message' => __( 'The response from SmilePay could not be parsed', 'moksa-for-woocommerce' ),
				'data'    => [ 'raw' => $raw ],
			];
		}
