...
final class InvoiceChannelOps {
	...
	private static function providers(): array {
		return array(
			'ecpay'    => __( 'ECPay e-invoice', 'moksa-for-woocommerce' ),
			'ezpay'    => __( 'ezPay e-invoice', 'moksa-for-woocommerce' ),
			'smilepay' => __( 'SmilePay e-invoice', 'moksa-for-woocommerce' ),
			'paynow'   => __( 'PayNow e-invoice', 'moksa-for-woocommerce' ),
			'amego'    => __( 'Amego e-invoice', 'moksa-for-woocommerce' ),
		);
	}

	private static function channels(): array {
		return array(
			'member' => array( 'channel_member', __( 'Member carrier', 'moksa-for-woocommerce' ) ),
			'mobile' => array( 'channel_mobile', __( 'Mobile barcode', 'moksa-for-woocommerce' ) ),
			'cert'   => array( 'channel_cert', __( 'Citizen Digital Certificate', 'moksa-for-woocommerce' ) ),
			'paper'  => array( 'channel_paper', __( 'Paper invoice', 'moksa-for-woocommerce' ) ),
			'donate' => array( 'allow_donate', __( 'Donation', 'moksa-for-woocommerce' ) ),
			'b2b'    => array( 'allow_b2b', __( 'Tax ID (business)', 'moksa-for-woocommerce' ) ),
		);
	}
	...
	public static function list_channels( $input ): array {
		...
		if ( '' === $provider ) {
			return array(
				'channels' => array(),
				'message'  => __( 'That e-invoice module could not be found.', 'moksa-for-woocommerce' ),
			);
		}
		$ch   = self::channels();
		$rows = array();
	...
	public static function toggle_prepare( $args ) {
		...
		if ( '' === $provider ) {
			return new \WP_Error( 'moksafowo_ai_bad_provider', __( 'That e-invoice module could not be found. The available ones are ECPay, ezPay, SmilePay, PayNow and Amego.', 'moksa-for-woocommerce' ) );
		}
		...
		if ( empty( $matched ) ) {
			return new \WP_Error( 'moksafowo_ai_no_match', __( 'No matching issuing option was found. The available ones are member carrier, mobile barcode, Citizen Digital Certificate, paper, donation and tax ID.', 'moksa-for-woocommerce' ) );
		}
		...
		$summary = sprintf(
			...
			__( '%1$s the %2$s issuing options: %3$s.', 'moksa-for-woocommerce' ),
	...
	public static function toggle_apply( array $params ) {
		...
		return sprintf(
			/* translators: 1: enable/disable, 2: provider, 3: channel labels */
			__( '✅ The %2$s issuing options are now %1$s: %3$s.', 'moksa-for-woocommerce' ),
			$enable ? __( 'Enable', 'moksa-for-woocommerce' ) : __( 'disabled', 'moksa-for-woocommerce' ),
