		...
		if ( in_array( $rtn_code, [ 1, 2, 10100073 ], true ) ) {
			if ( 1 === $rtn_code ) {
				if ( ! $order->is_paid() ) {
					...
					$order->add_order_note(
						...
						/* translators: %s: 中文化的付款方式名稱 */
							__( 'Payment method: %s', 'moksa-for-woocommerce' ),
				...
				$note = sprintf(
					...
					__( 'ECPay issued a payment code (%1$s, awaiting payment. Status code %2$s)', 'moksa-for-woocommerce' ),
					self::payment_type_label( (string) $payment_type ),
					(string) $rtn_code
				);
			...
			$note = sprintf(
				...
				__( 'ECPay payment failed: %1$s (status code %2$s)', 'moksa-for-woocommerce' ),
