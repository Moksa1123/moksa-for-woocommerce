...
final class OrderMetaBox {
	...
	public static function add_card( array $cards, \WC_Order $order ): array {
		...
		ob_start();
		echo '<p><strong>' . esc_html__( 'Payment method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( self::pay_type_label( $pay_type ) ) . '</p>';
		if ( '' !== $trade_no ) {
			echo '<p><strong>' . esc_html__( 'ECPay transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $trade_no ) . '</p>';
		}
		if ( '' !== $mtn ) {
			echo '<p><strong>' . esc_html__( 'Merchant transaction ID:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $mtn ) . '</p>';
		}
		if ( '' !== $card_last4 ) {
			echo '<p><strong>' . esc_html__( 'Last four card digits:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $card_last4 ) . '</p>';
		}
		if ( '' !== $pay_date ) {
			echo '<p><strong>' . esc_html__( 'Paid at:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $pay_date ) . '</p>';
		}
		...
		if ( '' !== $atm_acct ) {
			...
			echo '<p><strong>' . esc_html__( 'Payment code:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $cvs_no ) . '</span></p>';
			if ( '' !== $expire ) {
				echo '<p><strong>' . esc_html__( 'Pay before:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $expire ) . '</p>';
			}
		} elseif ( '' !== $barcode1 ) {
