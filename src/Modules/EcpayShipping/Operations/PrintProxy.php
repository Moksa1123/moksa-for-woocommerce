	...
	public static function add_print_actions( array $actions, \WC_Order $order ): array {
		...
		foreach ( $modes as $mode => $tone ) {
			...
			$actions[ 'moksafowo_ecpay_print_' . $tone ] = [
				'url'    => $url,
				'name'   => 'a4' === $tone ? __( 'Print shipping label (A4)', 'moksa-for-woocommerce' ) : __( 'Print shipping label (A6)', 'moksa-for-woocommerce' ),
...
JS;
	...
	public static function handle_quick(): void {
		...
		if ( empty( $buckets ) ) {
			wp_die( esc_html__( 'The shipment details are incomplete.', 'moksa-for-woocommerce' ), '', 400 );
		}
		...
		if ( count( $buckets ) > 1 ) {
			...
			$paragraphs[] = sprintf( __( 'This order uses %1$d different carriers, so %2$d print windows will open.', 'moksa-for-woocommerce' ), count( $buckets ), count( $buckets ) );
			$paragraphs[] = __( 'If the browser blocks the pop-ups, allow them and click Print shipping label again.', 'moksa-for-woocommerce' );
		}
		...
		Interstitial::render(
			__( 'Print shipping label', 'moksa-for-woocommerce' ),
			/* translators: %d: total number of shipping labels across all subtypes */
			sprintf( __( 'Printing %d shipping labels…', 'moksa-for-woocommerce' ), array_sum( array_map( 'count', $buckets ) ) ),
			$paragraphs,
			$forms,
	...
	public static function handle(): void {
		...
		if ( empty( $ids ) || '' === $subtype ) {
			wp_die( esc_html__( 'A required parameter is missing.', 'moksa-for-woocommerce' ), '', 400 );
		}
		...
		if ( $is_forward ) {
			...
			echo '<!DOCTYPE html><meta charset="utf-8"><div style="font-family:-apple-system,\'Microsoft JhengHei\',sans-serif;text-align:center;margin-top:18vh;color:#1d2327;font-size:16px">' . esc_html__( 'Generating the shipping label…', 'moksa-for-woocommerce' ) . '</div>';
