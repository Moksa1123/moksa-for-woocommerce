...
final class LinepayBlocksMethod extends AbstractPaymentMethodType {
	...
	public function get_payment_method_script_handles() {
		...
		$js_path = MOWC_PLUGIN_DIR . 'src/Modules/Linepay/assets/js/blocks/linepay.js';
		$ver     = file_exists( $js_path ) ? (string) filemtime( $js_path ) : MOWC_VERSION;
		wp_register_script(
			'mo-linepay-blocks',
			MOWC_PLUGIN_URL . 'src/Modules/Linepay/assets/js/blocks/linepay.js',
			array(
				'wc-blocks-registry',
				'wc-settings',
				'wp-element',
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'mo-linepay-blocks', 'mo-ectools' );
		}

		return array( 'mo-linepay-blocks' );
	}
