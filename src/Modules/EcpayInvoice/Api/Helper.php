...
namespace Moksafowo\Modules\EcpayInvoice\Api;
...
final class Helper extends AbstractCredentialHelper {
	...
	public static function post( string $path, array $data ): array {
		...
		if ( ! is_array( $decoded ) ) {
			return [
				'ok'      => false,
				'message' => 'decoded data not array',
				'raw'     => $plain_data,
			];
		}
	...
	public static function check_barcode( string $barcode ): array {
		...
		if ( ! preg_match( '#^/[0-9a-zA-Z+\-.]{7}$#', $barcode ) ) {
			return [
				...
				'exists'  => false,
				'code'    => '1041',
				'message' => __( '手機條碼格式錯誤（需 / 開頭 + 7 碼字母/數字/+ - .）。', 'mo-ectools' ),
			];
		}
		...
		// RtnCode=9000001 means MoF API under maintenance — pass through to avoid blocking orders.
		$inner_code = (int) ( $result['data']['RtnCode'] ?? 0 );
		if ( 9000001 === $inner_code ) {
			self::log( 'CheckBarcode MoF under maintenance — skipping validation', [ 'barcode' => $barcode ] );
			return [
				'ok'      => true,
				'exists'  => true,
				'code'    => '1042',
				'message' => __( '財政部系統維護中，已跳過載具驗證。', 'mo-ectools' ),
			];
		}
		...
		if ( ! $result['ok'] ) {
			return [
				...
				'exists'  => false,
				'code'    => '1049',
				'message' => $result['message'] ?? __( '驗證失敗。', 'mo-ectools' ),
			];
		}
		...
		if ( 'Y' !== $is_exist ) {
			return [
				...
				'exists'  => false,
				'code'    => '1043',
				'message' => __( '此手機條碼不存在，請確認是否輸入正確。', 'mo-ectools' ),
			];
		}
	...
	public static function check_love_code( string $love_code ): array {
		...
		if ( ! preg_match( '#^([xX][0-9]{2,6}|[0-9]{3,7})$#', $love_code ) ) {
			return [
				...
				'exists'  => false,
				'code'    => '1021',
				'message' => __( '愛心碼格式錯誤（需 3-7 碼數字，或 X + 2-6 碼數字）。', 'mo-ectools' ),
			];
		}

		$result = self::post(
			'/B2CInvoice/CheckLoveCode',
			[
				'MerchantID' => self::merchant_id(),
				'LoveCode'   => $love_code,
			]
		...
		$inner_code = (int) ( $result['data']['RtnCode'] ?? 0 );
		if ( 9000001 === $inner_code ) {
			self::log( 'CheckLoveCode MoF under maintenance — skipping validation', [ 'love_code' => $love_code ] );
			return [
				'ok'      => true,
				'exists'  => true,
				'code'    => '1022',
				'message' => __( '財政部系統維護中，已跳過愛心碼驗證。', 'mo-ectools' ),
			];
		}
		...
		if ( 'Y' !== $is_exist ) {
			return [
				...
				'exists'  => false,
				'code'    => '1023',
				'message' => __( '此愛心碼不存在，請確認是否輸入正確。', 'mo-ectools' ),
			];
		}
