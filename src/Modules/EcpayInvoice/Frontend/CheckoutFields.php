...
final class CheckoutFields {
	...
	public static function init(): void {
		...
		CheckoutAssets::register( 'mo_ecpay_invoice_default_donate_code' );
	}

	public static function render_classic(): void {
		$allow_donate = 'yes' === get_option( 'mo_ecpay_invoice_allow_donate', 'yes' );
		$allow_b2b    = 'yes' === get_option( 'mo_ecpay_invoice_allow_b2b', 'yes' );
		...
		if ( $allow_b2b ) {
			$type_options['b2b'] = __( '公司', 'mo-ectools' );
		}
		if ( $allow_donate ) {
			$type_options['b2c_donate'] = __( '捐贈', 'mo-ectools' );
		}
		...
		echo '<div class="mo-invoice-section" style="clear:both;overflow:hidden;display:block;">';
		echo '<h3>' . esc_html__( '電子發票', 'mo-ectools' ) . '</h3>';
		woocommerce_form_field( 'mo_invoice_type', [
			'type'    => 'select',
		...
		woocommerce_form_field( 'mo_invoice_carrier_type', [
			'type'    => 'select',
			'label'   => __( '載具類型', 'mo-ectools' ),
			'class'   => [ 'form-row-wide', 'mo-invoice-b2c-only' ],
			'options' => [
			...
			'default' => (string) get_option( 'mo_ecpay_invoice_default_carrier', 'member' ),
		] );

		woocommerce_form_field( 'mo_invoice_carrier_num', [
			'type'  => 'text',
			'label' => __( '載具編號', 'mo-ectools' ),
			'class' => [ 'form-row-wide', 'mo-invoice-carrier-num-row' ],
		] );

		if ( $allow_b2b ) {
			woocommerce_form_field( 'mo_invoice_buyer_ubn', [
				'type'  => 'text',
				'label' => __( '統一編號', 'mo-ectools' ),
				'class' => [ 'form-row-first', 'mo-invoice-b2b-only' ],
			] );
			woocommerce_form_field( 'mo_invoice_buyer_name', [
				'type'  => 'text',
				'label' => __( '公司名稱', 'mo-ectools' ),
				'class' => [ 'form-row-last', 'mo-invoice-b2b-only' ],
			] );
		}

		if ( $allow_donate ) {
			$default_code = preg_replace( '/[^0-9]/', '', (string) get_option( 'mo_ecpay_invoice_default_donate_code', '' ) );
			$default_code = substr( (string) $default_code, 0, 7 );
			woocommerce_form_field( 'mo_invoice_love_code', [
				'type'        => 'text',
				'label'       => __( '愛心碼', 'mo-ectools' ),
				'class'       => [ 'form-row-wide', 'mo-invoice-donate-only' ],
				'default'     => $default_code,
				'placeholder' => '' !== $default_code ? '' : __( '3-7 碼數字', 'mo-ectools' ),
			] );
		}
		echo '</div>'; // .mo-invoice-section
	}
	...
	public static function validate_ubn_block( $value ) {
		...
		if ( ! Ubn::is_valid( $value ) ) {
			return new \WP_Error( 'mo_invoice_ubn_format', __( '統一編號格式或檢查碼不正確。', 'mo-ectools' ) );
		}
		return null;
	}
	...
	public static function validate_love_code_block( $value ) {
		...
		if ( ! preg_match( '/^([xX]\d{2,6}|\d{3,7})$/', $value ) ) {
			return new \WP_Error( 'mo_invoice_love_code_format', __( '愛心碼格式錯誤（3-7 碼數字）。', 'mo-ectools' ) );
		}
		return null;
	}
	...
	public static function validate_format( $data, $errors ): void {
		...
		$type = self::field_value( $data, [ 'mo_invoice_type', '_mowp/invoice-type', 'mowp/invoice-type' ] );

		if ( 'b2b' === $type ) {
			$ubn = self::field_value( $data, [ 'mo_invoice_buyer_ubn', '_mowp/invoice-buyer-ubn', 'mowp/invoice-buyer-ubn' ] );
			if ( ! Ubn::is_valid( $ubn ) ) {
				$errors->add( 'mo_invoice_ubn', __( '統一編號格式或檢查碼不正確。', 'mo-ectools' ) );
			}
		}

		if ( 'b2c_donate' === $type ) {
			$code = self::field_value( $data, [ 'mo_invoice_love_code', '_mowp/invoice-love-code', 'mowp/invoice-love-code' ] );
			if ( ! preg_match( '/^([xX]\d{2,6}|\d{3,7})$/', $code ) ) {
				$errors->add( 'mo_invoice_love_code', __( '愛心碼格式錯誤（3-7 碼數字）。', 'mo-ectools' ) );
			}
		}

		if ( 'b2c_carrier' === $type ) {
			$carrier     = self::field_value( $data, [ 'mo_invoice_carrier_type', '_mowp/invoice-carrier-type', 'mowp/invoice-carrier-type' ] );
			$carrier_num = self::field_value( $data, [ 'mo_invoice_carrier_num', '_mowp/invoice-carrier-num', 'mowp/invoice-carrier-num' ] );
			if ( 'mobile' === $carrier && ! preg_match( '/^\/[0-9A-Z+\-.]{7}$/', $carrier_num ) ) {
				$errors->add( 'mo_invoice_carrier_mobile', __( '手機條碼格式錯誤（/ 開頭加 7 碼大寫英數）。', 'mo-ectools' ) );
			}
			if ( 'cert' === $carrier && ! preg_match( '/^[A-Z]{2}\d{14}$/', $carrier_num ) ) {
				$errors->add( 'mo_invoice_carrier_cert', __( '自然人憑證格式錯誤（2 大寫字母 + 14 碼數字）。', 'mo-ectools' ) );
			}
		}
	}
	...
	public static function validate_carrier_via_api( $data, $errors ): void {
		...
		// 商家 opt-out
		if ( 'yes' !== get_option( 'mo_ecpay_invoice_carrier_api_check', 'yes' ) ) {
			return;
		}

		// 撈 invoice_type — Classic 用 mo_invoice_type，Block 走 additional fields key
		// $data 是 WC checkout 的 cleaned post data；$_POST 是原始的（Block fallback）
		$type    = self::field_value( $data, [ 'mo_invoice_type', '_mowp/invoice-type', 'mowp/invoice-type' ] );
		$carrier = self::field_value( $data, [ 'mo_invoice_carrier_type', '_mowp/invoice-carrier-type', 'mowp/invoice-carrier-type' ] );
		$carrier_num = self::field_value( $data, [ 'mo_invoice_carrier_num', '_mowp/invoice-carrier-num', 'mowp/invoice-carrier-num' ] );
		$love_code   = self::field_value( $data, [ 'mo_invoice_love_code', '_mowp/invoice-love-code', 'mowp/invoice-love-code' ] );
		...
		if ( 'b2c_carrier' === $type && 'mobile' === $carrier && '' !== $carrier_num ) {
			...
			if ( $result['ok'] && ! $result['exists'] ) {
				$errors->add( 'mo_ecpay_invoice_barcode_invalid', $result['message'] );
			}
		...
		if ( 'b2c_donate' === $type && '' !== $love_code ) {
			...
			if ( $result['ok'] && ! $result['exists'] ) {
				$errors->add( 'mo_ecpay_invoice_love_code_invalid', $result['message'] );
			}
			if ( ! $result['ok'] && in_array( $result['code'], [ '1020', '1021' ], true ) ) {
				$errors->add( 'mo_ecpay_invoice_love_code_format', $result['message'] );
			}
		}
	}
	...
	public static function save_classic( \WC_Order $order, array $data ): void {
		// phpcs:disable WordPress.Security.NonceVerification.Missing,WordPress.Security.NonceVerification.Recommended -- WC checkout submit nonce 'woocommerce-process-checkout-nonce' verified by WC core in WC_Checkout::process_checkout() before this callback fires.
		$keys = [
			'mo_invoice_type'         => Keys::INVOICE_TYPE,
			'mo_invoice_carrier_type' => Keys::INVOICE_CARRIER_TYPE,
			'mo_invoice_carrier_num'  => Keys::INVOICE_CARRIER_NUM,
			'mo_invoice_buyer_ubn'    => Keys::INVOICE_BUYER_UBN,
			'mo_invoice_buyer_name'   => Keys::INVOICE_BUYER_NAME,
			'mo_invoice_love_code'    => Keys::INVOICE_LOVE_CODE,
		];
	...
	public static function register_block_fields(): void {
		...
		$type_options = [
			[ 'value' => 'b2c_carrier', 'label' => __( '個人', 'mo-ectools' ) ],
		];
		if ( $allow_b2b ) {
			$type_options[] = [ 'value' => 'b2b', 'label' => __( '公司', 'mo-ectools' ) ];
		}
		if ( $allow_donate ) {
			$type_options[] = [ 'value' => 'b2c_donate', 'label' => __( '捐贈', 'mo-ectools' ) ];
		}
		...
		woocommerce_register_additional_checkout_field( [
			...
			'type'     => 'select',
			'options'  => [
				[ 'value' => 'member', 'label' => __( '會員載具', 'mo-ectools' ) ],
				[ 'value' => 'mobile', 'label' => __( '手機條碼', 'mo-ectools' ) ],
				[ 'value' => 'cert',   'label' => __( '自然人憑證', 'mo-ectools' ) ],
				[ 'value' => 'paper',  'label' => __( '紙本', 'mo-ectools' ) ],
			],
			'required' => false,
		] );
		woocommerce_register_additional_checkout_field( [
