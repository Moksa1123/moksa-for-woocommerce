	...
	public function get_settings(): array {
		return [
			[
				...
				'type'  => 'title',
				'desc'  => __( '從綠界發票後台「廠商專區 → 系統識別資料」複製過來。發票帳號跟金流 / 物流是<strong>獨立申請</strong>。', 'moksa-for-woocommerce' ),
			...
			[
				...
				'type'    => 'checkbox',
				'default' => 'yes',
				'desc'    => __( '上線前用，勾選後，所有發票走測試環境不會真開立。上線後請取消勾選。', 'moksa-for-woocommerce' ),
			],
			[
			...
			[
				'title' => __( '開立規則', 'moksa-for-woocommerce' ),
				'type'  => 'title',
				'id'    => 'moksafowo_ecpay_invoice_behavior_section',
			],
			[
				'title'             => __( '發票編號前綴', 'moksa-for-woocommerce' ),
				'id'                => 'moksafowo_ecpay_invoice_prefix',
				'type'              => 'text',
				'default'           => '',
				'desc_tip'          => __( '最多 5 字元（限英數字）。同一個綠界發票帳號跨多店時，用前綴避免發票號撞號。', 'moksa-for-woocommerce' ),
				'custom_attributes' => [
			...
			[
				'title'   => __( '何時開立', 'moksa-for-woocommerce' ),
				'id'      => 'moksafowo_ecpay_invoice_issue_when',
				'type'    => 'select',
				'options' => [
					'paid'      => __( '付款完成立即開立（建議）', 'moksa-for-woocommerce' ),
					'completed' => __( '訂單完成（出貨後）才開立', 'moksa-for-woocommerce' ),
					'manual'    => __( '只手動開立（後台按鈕觸發）', 'moksa-for-woocommerce' ),
				],
				'default' => 'paid',
			],
			[
				'title'             => __( '延後開立天數', 'moksa-for-woocommerce' ),
				'id'                => 'moksafowo_ecpay_invoice_delay_days',
				'type'              => 'number',
				'default'           => 0,
				'desc_tip'          => __( '達到「何時開立」條件後再延幾天才實際開立。預設 0 = 立即。常見：訂單完成後 N 天才開（避免取消退款）。', 'moksa-for-woocommerce' ),
				'custom_attributes' => [
					'min'  => 0,
					'max'  => 30,
					'step' => 1,
			...
			[
				...
				'default' => 'manual',
				'options' => [
					'manual'      => __( '手動 — 商家自己進訂單編輯頁按「作廢發票」', 'moksa-for-woocommerce' ),
					'auto_cancel' => __( '自動 — 訂單變取消 / 退款 / 失敗時 2 分鐘後自動作廢', 'moksa-for-woocommerce' ),
				],
				'desc'    => __( '改成「自動」後，訂單取消 / 退款 / 失敗時會自動作廢該訂單已開立的發票，避免忘記作廢造成稅務問題。', 'moksa-for-woocommerce' ),
			],
			[
				'title'   => __( '載具 / 愛心碼真驗', 'moksa-for-woocommerce' ),
				'id'      => 'moksafowo_ecpay_invoice_carrier_api_check',
				'type'    => 'checkbox',
				'default' => 'yes',
				'desc'    => __( '結帳時即時驗證顧客輸入的手機條碼 / 愛心碼是否真實存在，避免偽造載具導致發票開立失敗。', 'moksa-for-woocommerce' ),
			],
			[
				...
				'type'    => 'checkbox',
				'default' => 'yes',
				'desc'    => __( '提供「捐贈發票」選項（消費者輸入 3-7 碼捐贈碼）。', 'moksa-for-woocommerce' ),
			],
			[
			...
			[
				...
				'type'    => 'checkbox',
				'default' => 'yes',
				'desc'    => __( '提供統一編號欄位，公司行號可開三聯式發票。', 'moksa-for-woocommerce' ),
			],
			[
				'title'         => __( '開放的個人載具', 'moksa-for-woocommerce' ),
				'desc'          => __( '會員載具（雲端，免輸入）', 'moksa-for-woocommerce' ),
			...
			[
				'desc'          => __( '紙本', 'moksa-for-woocommerce' ),
			...
			[
				...
				'type'     => 'select',
				'options'  => [
					'member' => __( '會員載具（雲端，免輸入）', 'moksa-for-woocommerce' ),
					'mobile' => __( '手機條碼（消費者輸入 8 碼）', 'moksa-for-woocommerce' ),
					'cert'   => __( '自然人憑證（消費者輸入 16 碼）', 'moksa-for-woocommerce' ),
					'paper'  => __( '紙本（不發載具）', 'moksa-for-woocommerce' ),
				],
				'default'  => 'member',
			...
			[
				...
				'type'    => 'checkbox',
				'default' => 'no',
				'desc'    => __( '排查發票異常時開啟。', 'moksa-for-woocommerce' ),
			],
			[
				'type' => 'sectionend',
				'id'   => 'moksafowo_ecpay_invoice_debug_section',
