...
use Moksafowo\Modules\Paynow\Api\Helper;
use Moksafowo\Modules\Paynow\Api\PaymentRequest;
...
abstract class AbstractPaynowGateway extends AbstractMowcGateway {
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( $order instanceof \WC_Order ) {
			$order->add_order_note(
				...
				/* translators: 1: amount, 2: reason */
					__( 'PayNow refunds are made in the PayNow dashboard (NT$%1$s) — %2$s', 'moksa-for-woocommerce' ),
		...
		return new \WP_Error(
			...
			__( 'PayNow refunds are made in the PayNow dashboard, since automatic refunds can get an account suspended.', 'moksa-for-woocommerce' )
		);
	}
}
