...
final class PaymentRequest {
	...
	public static function render_form( array $params ): string {
		...
		<form method="post" id="moksafowo-paynow-form" accept-charset="UTF-8" action="<?php echo esc_url( $action ); ?>">
			...
			<?php endforeach; ?>
			<button type="submit" id="moksafowo-paynow-submit" class="button alt"><?php esc_html_e( 'Pay with PayNow', 'moksa-for-woocommerce' ); ?></button>
		</form>
		<?php
	...
	private static function sanitize_name( string $name ): string {
		...
		$clean = trim( $clean );
		if ( '' === $clean ) {
			$clean = __( 'Customer', 'moksa-for-woocommerce' );
		}
	...
	private static function clamp_order_info( string $info ): string {
		...
		if ( mb_strlen( $info ) < 5 ) {
			...
			$info = sprintf( __( '%s online order', 'moksa-for-woocommerce' ), get_bloginfo( 'name' ) );
		}
