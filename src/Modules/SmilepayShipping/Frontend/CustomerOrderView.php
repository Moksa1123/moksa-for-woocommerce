...
final class CustomerOrderView {
				...
				foreach ( $records as $r ) :
					...
					<div class="mo-shipping-card__row">
						...
						<span class="mo-shipping-card__value" style="display:flex;flex-direction:column;gap:6px;">
							<?php if ( '' !== $primary ) : ?>
								<span class="mo-shipping-card__code"><?php echo esc_html( $primary ); ?></span>
							<?php endif; ?>
	...
	private static function carrier_title( string $method_id ): string {
		$map = [
			...
			'mo_smilepay_shipping_tcat_normal'  => __( '速買配 — 黑貓宅配 (常溫)', 'mo-ectools' ),
			'mo_smilepay_shipping_tcat_refrige' => __( '速買配 — 黑貓宅配 (冷藏)', 'mo-ectools' ),
			'mo_smilepay_shipping_tcat_freeze'  => __( '速買配 — 黑貓宅配 (冷凍)', 'mo-ectools' ),
		];
