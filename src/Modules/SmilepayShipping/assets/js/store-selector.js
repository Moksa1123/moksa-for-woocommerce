( function ( $ ) {
	...
	function injectButton() {
		...
		// 移除舊的
		$( '.moksafowo-smilepay-shipping-store' ).remove();
		...
		const $box = $( '<div class="moksafowo-smilepay-shipping-store" style="margin:8px 0;padding:10px;border:1px solid #c0c0c0;border-radius:4px;background:#f8f9fa;"></div>' );
		$box.append( $( '<div></div>' ).text( sel ).css( { 'font-size': '13px', 'margin-bottom': '6px' } ) );
		$box.append(
			$( '<button type="button" class="moksafowo-smilepay-shipping-store__btn button"></button>' )
				.text( btnLabel )
				.on( 'click', function ( e ) {
					e.preventDefault();
