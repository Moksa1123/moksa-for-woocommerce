...
final class OrderMetaBox {
	...
	public static function ajax_create_shipment(): void {
		...
		if ( ! empty( $existing ) ) {
			...
			if ( ! $force ) {
				wp_send_json_error(
					[
						'code'    => 'existing',
						'message' => __( 'This order already has shipments. Only the temperature zones without one will be created, and nothing is booked twice. To rebuild them all, delete the existing records first. Continue?', 'moksa-for-woocommerce' ),
					]
				);
			}
		}
			...
			wp_send_json_success(
				[
					'message' => sprintf(
						...
						__( 'SmilePay shipment created (smseid=%1$s, number=%2$s)', 'moksa-for-woocommerce' ),
						$result['smseid'] ?? '-',
		...
		wp_send_json_error( [ 'message' => $result['message'] ?? __( 'Something went wrong', 'moksa-for-woocommerce' ) ] );
	}
		...
		<div class="moksafowo-smilepay-shipping-meta"
			...
			<?php if ( ! empty( $records ) ) : ?>
				<div class="moksafowo-smilepay-records" style="display:flex;flex-direction:column;gap:8px;margin:0 0 8px;">
					...
					<div style="background:#f6f7f7;border:1px solid #dcdcde;border-radius:4px;padding:10px 12px;font-size:12px;">
						...
						<?php if ( '' !== $pay_no ) : ?>
							<p style="margin:.2em 0;"><strong><?php esc_html_e( 'Shipping number:', 'moksa-for-woocommerce' ); ?></strong><span style="font-family:monospace;word-break:break-all;"><?php echo esc_html( $pay_no ); ?></span></p>
						<?php endif; ?>
						<?php if ( '' !== $track_num ) : ?>
							<p style="margin:.2em 0;"><strong><?php esc_html_e( 'Waybill number:', 'moksa-for-woocommerce' ); ?></strong><span style="font-family:monospace;word-break:break-all;"><?php echo esc_html( $track_num ); ?></span></p>
						<?php endif; ?>
						<?php if ( '' !== $created_at ) : ?>
			...
			<div class="moksafowo-smilepay-shipping-actions">
				<button type="button"
					...
					<?php echo esc_html( empty( $records ) ? __( 'Create shipment', 'moksa-for-woocommerce' ) : __( 'Recreate shipment', 'moksa-for-woocommerce' ) ); ?>
				</button>
			</div>
		</div>
		<?php
