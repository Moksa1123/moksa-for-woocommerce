...
final class CreateOrderUnified {
	...
	public static function run( \WC_Order $order ): array {
		...
		if ( null === $method ) {
			return [
				'ok'      => false,
				'message' => __( 'This is not a PAYUNi unified method order.', 'moksa-for-woocommerce' ),
			];
		}
		...
		if ( empty( $created ) && empty( $existing ) ) {
			...
			$order->add_order_note( __( 'None of the PAYUNi shipments could be created:', 'moksa-for-woocommerce' ) . $msg );
			$order->save();
			return [
				'ok'      => false,
				'message' => $msg,
		...
		if ( count( $created ) > 1 ) {
			...
			$order->add_order_note(
				...
				/* translators: 1: count, 2: list */
					__( 'PAYUNi created the T-Cat shipments, split into %1$d parcels by temperature zone: %2$s', 'moksa-for-woocommerce' ),
					count( $created ),
					"\n" . implode( "\n", $lines )
				)
			);
			...
			$order->add_order_note(
				...
				/* translators: 1: temp label, 2: PAYUNi ship trade no, 3: pickup no */
					__( 'PAYUNi created the T-Cat home delivery shipment — %1$s (shipping ID %2$s, pickup number %3$s)', 'moksa-for-woocommerce' ),
					ProductTemp::label( (int) $r['temp'] ),
					(string) $r['ship_trade_no'],
					(string) $r['odno']
				)
