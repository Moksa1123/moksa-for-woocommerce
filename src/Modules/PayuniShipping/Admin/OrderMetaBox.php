...
class OrderMetaBox {
	...
	private static function card_html( \WC_Order $order ): string {
		...
		echo '<tr><th style="text-align:left;"><div id="order-id" data-order-id="' . esc_attr( (string) $oid ) . '">' . esc_html__( 'Shipping transaction ID', 'moksa-for-woocommerce' ) . '</div></th><td>' . esc_html( $order->get_meta( OrderMeta::ShipTradeNo ) ) . '</td></tr>';

		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $provider_query_html built locally with esc_url applied
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Tracking number', 'moksa-for-woocommerce' ) . '</th><td style="word-break:break-all;">' . esc_html( $shipping_no ) . $provider_query_html . '</td></tr>';

		if ( $ship_type === ShipType::SEVEN ) {
			echo '<tr><th style="text-align:left;">' . esc_html__( 'Store number', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $partner_id ) . '</td></tr>';
			echo '<tr><th style="text-align:left;">' . esc_html__( 'Order number', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $odno ) . '</td></tr>';

			if ( $lgs_type === LgsType::C2C ) {
				echo '<tr><th style="text-align:left;">' . esc_html__( 'Verification code', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $validation_no ) . '</td></tr>';
			}
		}
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Shipping type', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( ShipType::get_name( $ship_type ) ) . '</td></tr>';
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Delivery method', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( LgsType::get_name( $lgs_type ) ) . '</td></tr>';
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Product type', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( GoodsType::get_name( $goods_type ) ) . '</td></tr>';
		...
		if ( $ship_type === ShipType::TCAT ) {
			...
			echo '<tr><th style="text-align:left;">' . esc_html__( 'Parcel size', 'moksa-for-woocommerce' ) . '</th><td>';
		...
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Service type', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( ServiceType::get_name( $service_type ) ) . '</td></tr>';

		if ( $order->get_payment_method() === 'cod' || $service_type === ServiceType::COD ) {
			echo '<tr><th style="text-align:left;">' . esc_html__( 'Amount to collect', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $trade_amt ) . '</td></tr>';
		}

		echo '<tr><th style="text-align:left;">' . esc_html__( 'Shipping status', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $ship_status ) . '</td></tr>';
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Status description', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $ship_status_desc ) . '</td></tr>';
		echo '<tr><th style="text-align:left;">' . esc_html__( 'Status updated', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $ship_status_time ) . '</td></tr>';

		if ( $ship_type === ShipType::TCAT ) {
			echo '<tr><th style="text-align:left;">' . esc_html__( 'Printed at', 'moksa-for-woocommerce' ) . '</th><td>' . esc_html( $print_date ) . '</td></tr>';
		}

		if ( ! empty( $shipping_no ) && $ship_type === ShipType::TCAT ) {
			$label_btn = '<button class="button print-label" data-id="' . esc_attr( (string) $oid ) . '" data-service="' . esc_attr( (string) $ship_type ) . '" data-action="moksafowo_payuni_shipping_download_label">' . esc_html__( 'Download label', 'moksa-for-woocommerce' ) . '</button>';
		} else {
