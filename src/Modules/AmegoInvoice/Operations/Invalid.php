...
final class Invalid {
	...
	public static function run( \WC_Order $order, string $reason = '' ): array {
		...
		if ( '' === $invoice_no || 'zero' === $invoice_no || 'negative' === $invoice_no ) {
			return [ 'ok' => false, 'message' => __( '此訂單沒有可作廢的 Amego 發票。', 'mo-ectools' ) ];
		}
		...
		if ( ! $resp['ok'] ) {
			$msg = sprintf(
				...
				__( 'Amego 發票作廢失敗 (#%1$s)：%2$s', 'mo-ectools' ),
				$invoice_no,
				$resp['message']
			);
		...
		$order->add_order_note( sprintf(
			...
			__( 'Amego 發票已作廢 (#%1$s)：%2$s', 'mo-ectools' ),
			$invoice_no,
