...
final class Allowance {
	...
	public static function run( \WC_Order $order, int $amount ): array {
		...
		if ( '' === $inv || 'zero' === $inv || 'negative' === $inv ) {
			return [
				'ok'      => false,
				'message' => __( 'This order has no ezPay invoice to raise an allowance against.', 'moksa-for-woocommerce' ),
			];
		}
		if ( $amount <= 0 ) {
			return [
		...
		if ( ! $result['ok'] ) {
			$order->add_order_note(
				sprintf(
				/* translators: %s: error */
					__( 'The ezPay allowance failed: %s', 'moksa-for-woocommerce' ),
					$result['message'] ?? ''
				)
			);
			$order->save();
		...
		$order->add_order_note(
			...
			/* translators: 1: allowance no, 2: amount */
				__( 'ezPay allowance %1$s was issued for NT$%2$d', 'moksa-for-woocommerce' ),
				$allowance_no,
				$amount
			)
		);
