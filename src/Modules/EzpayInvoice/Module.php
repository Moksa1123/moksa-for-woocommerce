...
namespace MoksaWeb\Mowc\Modules\EzpayInvoice;
...
final class Module extends AbstractModule {
	...
	public function label(): string {
		return __( 'ezPay 電子發票 — B2C / B2B / 手機條碼 / 自然人憑證 / 會員載具 / 捐贈', 'mo-ectools' );
	}
	...
	public function name(): string {
		return __( 'ezPay 電子發票', 'mo-ectools' );
	}

	public function tagline(): string {
		return __( 'B2C / B2B / 載具 / 捐贈 — 藍新旗下', 'mo-ectools' );
	}
	...
	public const ASYNC_ISSUE_HOOK = 'mo_ezpay_invoice_async_issue';
	...
	public function boot(): void {
		...
		$when = (string) get_option( 'mo_ezpay_invoice_issue_when', 'paid' );
		if ( 'paid' === $when ) {
		...
		if ( 'auto_cancel' === get_option( 'mo_ezpay_invoice_auto_cancel', 'manual' ) ) {
