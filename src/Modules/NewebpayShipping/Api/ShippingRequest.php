...
final class ShippingRequest {
	...
	public static function open_store_map( array $args ): array {
		...
		if ( ! $encrypt['ok'] ) {
			return [ 'ok' => false, 'message' => $encrypt['message'], 'api_url' => '', 'form_data' => [] ];
		}
		return [
			'ok'        => true,
	...
	public static function get_shipment_no( array $merchant_order_nos ): array {
		return self::call( self::PATH_GET_SHIPMENT_NO, [
	...
	public static function query_shipment( string $merchant_order_no ): array {
		return self::call( self::PATH_QUERY_SHIPMENT, [
	...
	public static function trace( string $merchant_order_no ): array {
		return self::call( self::PATH_TRACE, [
			'MerchantOrderNo' => $merchant_order_no,
			'TimeStamp'       => (string) time(),
		] );
	}
	...
	private static function call( string $path, array $args ): array {
		...
		if ( ! $encrypt['ok'] ) {
			return [ 'ok' => false, 'message' => $encrypt['message'] ];
		}
		...
		if ( ! is_array( $decoded ) ) {
			return [ 'ok' => false, 'message' => __( '藍新物流回傳格式錯誤', 'mo-ectools' ) ];
		}
		Helper::log( 'shipping API ' . $path, [ 'status' => (string) ( $decoded['Status'] ?? '' ), 'message' => (string) ( $decoded['Message'] ?? '' ) ] );

		// SUCCESS or numeric code: 1101+ = error
		if ( 'SUCCESS' !== ( $decoded['Status'] ?? '' ) ) {
			return [ 'ok' => false, 'message' => sprintf( '%s (%s)', $decoded['Message'] ?? '', $decoded['Status'] ?? '' ) ];
		}
	...
	private static function encrypt_args( array $args ): array {
		try {
			...
			return [ 'ok' => true, 'message' => 'OK', 'data' => $hex, 'hash' => $hash ];
		} catch ( \Throwable $e ) {
