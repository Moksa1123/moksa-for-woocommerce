...
final class PickupNotice {
	...
	public static function output(): void {
		...
		if ( empty( $orders ) ) {
			wp_die( esc_html__( 'There is no shipment to print, because the order has no pickup store.', 'moksa-for-woocommerce' ) );
		}
...
<head>
<meta charset="utf-8">
<title><?php esc_html_e( 'NewebPay shipment', 'moksa-for-woocommerce' ); ?></title>
		<?php wp_print_styles( 'moksafowo-newebpay-pickup' ); ?>
</head>
<body>
<div class="print-bar">
	<button type="button" id="moksafowo-pickup-print"><?php esc_html_e( 'Print', 'moksa-for-woocommerce' ); ?></button>
	<div class="note"><?php esc_html_e( 'Note: NewebPay does not supply a printed label, so this one is generated here. Attach it to the parcel as it is.', 'moksa-for-woocommerce' ); ?></div>
</div>
	...
	<div class="label">
		<div class="label__head">
			<h1><?php esc_html_e( 'NewebPay shipment', 'moksa-for-woocommerce' ); ?></h1>
			<div class="subline"><?php echo esc_html( $order->get_date_created()?->date( 'Y-m-d H:i' ) ?? '' ); ?></div>
		</div>
		<div class="row"><div class="label-key"><?php esc_html_e( 'Order number', 'moksa-for-woocommerce' ); ?></div><div class="label-val big">#<?php echo esc_html( (string) $order->get_id() ); ?></div></div>
		<div class="row"><div class="label-key"><?php esc_html_e( 'Sender', 'moksa-for-woocommerce' ); ?></div><div class="label-val"><?php echo esc_html( $shop_name ); ?></div></div>
			<?php if ( '' !== $shop_address ) : ?>
		<div class="row"><div class="label-key"><?php esc_html_e( 'Sender address', 'moksa-for-woocommerce' ); ?></div><div class="label-val"><?php echo esc_html( trim( $shop_postcode . ' ' . $shop_city . ' ' . $shop_address ) ); ?></div></div>
		<?php endif; ?>
		<div class="row"><div class="label-key"><?php esc_html_e( 'Recipient', 'moksa-for-woocommerce' ); ?></div><div class="label-val big"><?php echo esc_html( $customer ); ?></div></div>
		<div class="row"><div class="label-key"><?php esc_html_e( 'Recipient phone', 'moksa-for-woocommerce' ); ?></div><div class="label-val"><?php echo esc_html( $order->get_billing_phone() ); ?></div></div>
		<div class="store">
			<div class="store-id"><?php esc_html_e( 'Pickup store', 'moksa-for-woocommerce' ); ?>：<?php echo esc_html( $store_name ); ?> <span style="font-weight:normal;">(<?php echo esc_html( $store_id ); ?>)</span></div>
			<?php if ( '' !== $store_addr ) : ?>
		...
		<div class="items">
			<table>
				<thead><tr><th><?php esc_html_e( 'Items', 'moksa-for-woocommerce' ); ?></th><th style="width:50px;text-align:right;"><?php esc_html_e( 'Quantity', 'moksa-for-woocommerce' ); ?></th></tr></thead>
				<tbody>
		...
		<div class="row" style="margin-top:8px;border-top:1px solid #000;padding-top:6px;"><div class="label-key"><?php esc_html_e( 'Order total', 'moksa-for-woocommerce' ); ?></div><div class="label-val big">NT$<?php echo esc_html( (string) (int) $order->get_total() ); ?></div></div>
	</div>
		<?php endforeach; ?>
