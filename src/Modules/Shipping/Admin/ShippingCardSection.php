...
final class ShippingCardSection {
	...
	private static function render_payuni_shipping( \WC_Order $order ): string {
		...
		if ( empty( $records ) && '' === $trade_no && '' === $ship_no ) {
			ob_start();
			if ( '' !== $method_title ) {
				echo '<p><strong>' . esc_html__( 'Shipping method:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $method_title ) . '</p>';
			}
			echo '<p style="color:#646970;font-size:12px;">' . esc_html__( 'No PAYUNi shipment has been created yet.', 'moksa-for-woocommerce' ) . '</p>';
		...
		if ( ! empty( $records ) ) {
			...
			foreach ( $records as $r ) {
				...
				$type_label    = '1' === $ship_t ? __( '7-11', 'moksa-for-woocommerce' ) : ( '2' === $ship_t ? __( 'T-Cat home delivery', 'moksa-for-woocommerce' ) : '' );
				...
				if ( ! empty( $r['odno'] ) ) {
					echo '<p style="margin:.2em 0;"><strong>' . esc_html__( 'Carrier shipment number:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( (string) $r['odno'] ) . '</span></p>';
				}
				if ( ! empty( $r['validation_no'] ) ) {
					echo '<p style="margin:.2em 0;"><strong>' . esc_html__( 'Verification code:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( (string) $r['validation_no'] ) . '</span></p>';
				}
				if ( $amount > 0 ) {
					/* translators: %d: declared value amount in TWD */
					echo '<p style="margin:.2em 0;"><strong>' . esc_html__( 'Declared value:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( sprintf( __( 'NT$%d', 'moksa-for-woocommerce' ), $amount ) ) . '</p>';
					echo '<p style="margin:.2em 0;"><strong>' . esc_html__( 'Amount to collect:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $cod_label ) . '</p>';
				}
				if ( ! empty( $r['created_at'] ) ) {
					echo '<p style="margin:.2em 0;"><strong>' . esc_html__( 'Created:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( (string) $r['created_at'] ) . '</p>';
				}
		...
		if ( '' !== $trade_no ) {
			echo '<p><strong>' . esc_html__( 'PAYUNi shipping ID:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $trade_no ) . '</span></p>';
		}
		if ( '' !== $odno ) {
			echo '<p><strong>' . esc_html__( 'Carrier shipment number:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $odno ) . '</span></p>';
		}
		if ( '' !== $ship_no ) {
			echo '<p><strong>' . esc_html__( 'Shipment number:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $ship_no ) . '</span></p>';
		}
		if ( '' !== $trade_amt ) {
			$is_cod = '1' === $service_t;
			echo '<p><strong>' . esc_html__( 'Amount to collect:', 'moksa-for-woocommerce' ) . '</strong>';
			echo $is_cod
				/* translators: %d: cash-on-delivery amount in TWD */
				? esc_html( sprintf( __( 'NT$%d (cash on delivery)', 'moksa-for-woocommerce' ), (int) $trade_amt ) )
				: esc_html__( 'No', 'moksa-for-woocommerce' );
			echo '</p>';
		}
		if ( '' !== $status_d ) {
			echo '<p><strong>' . esc_html__( 'Shipping status:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $status_d );
			if ( '' !== $status ) {
		...
		if ( '' !== $status_t ) {
			echo '<p><strong>' . esc_html__( 'Status updated:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $status_t ) . '</p>';
		}
	...
	private static function render_newebpay_shipping( \WC_Order $order ): string {
		...
		if ( '' === $lgs_no && '' === $store_id ) {
			...
			echo esc_html__( 'No NewebPay shipping details yet. The customer can pick a store at checkout, or you can create the shipment yourself.', 'moksa-for-woocommerce' );
			echo '</p>';
		}
		if ( '' !== $lgs_no ) {
		...
		if ( '' !== $store_id || '' !== $store_name ) {
			echo '<p><strong>' . esc_html__( 'Pickup store:', 'moksa-for-woocommerce' ) . '</strong>';
			if ( '' !== $store_name ) {
		...
		if ( '' !== $store_addr ) {
			echo '<p><strong>' . esc_html__( 'Store address:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $store_addr ) . '</p>';
		}
		if ( '' !== $status ) {
			echo '<p><strong>' . esc_html__( 'Shipping status:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $status ) . '</p>';
		}

		echo '<p style="margin-top:10px;padding-top:8px;border-top:1px dashed #c0c0c0;">';
		if ( '' === $lgs_no && '' !== $store_id ) {
			echo '<button type="button" class="button button-primary moksafowo-newebpay-shipping-create" data-order="' . esc_attr( (string) $order_id ) . '" data-nonce="' . esc_attr( $nonce_create ) . '">' . esc_html__( 'Create NewebPay shipment', 'moksa-for-woocommerce' ) . '</button> ';
		}
		if ( '' !== $lgs_no ) {
			echo '<button type="button" class="button moksafowo-newebpay-shipping-query" data-order="' . esc_attr( (string) $order_id ) . '" data-nonce="' . esc_attr( $nonce_query ) . '">' . esc_html__( 'Check current status', 'moksa-for-woocommerce' ) . '</button> ';
			echo '<button type="button" class="button moksafowo-newebpay-shipping-trace" data-order="' . esc_attr( (string) $order_id ) . '" data-nonce="' . esc_attr( $nonce_trace ) . '">' . esc_html__( 'Shipment tracking', 'moksa-for-woocommerce' ) . '</button>';
		}
		echo '</p>';
	...
	private static function render_smilepay_shipping( \WC_Order $order ): string {
		...
		if ( '' !== $smseid ) {
			echo '<p><strong>' . esc_html__( 'SmilePay serial number:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $smseid ) . '</span></p>';
		}
		if ( '' !== $ship_type ) {
			echo '<p><strong>' . esc_html__( 'Shipping type:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $ship_type ) . ( '' !== $lgs_type ? ' (' . esc_html( $lgs_type ) . ')' : '' ) . '</p>';
		}
		if ( '' !== $store_id || '' !== $store_name ) {
			echo '<p><strong>' . esc_html__( 'Pickup store:', 'moksa-for-woocommerce' ) . '</strong>';
			if ( '' !== $store_name ) {
		...
		if ( '' !== $pay_no ) {
			echo '<p><strong>' . esc_html__( 'Pickup code:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $pay_no ) . '</span></p>';
		}
		if ( '' !== $track_no ) {
			echo '<p><strong>' . esc_html__( 'T-Cat waybill number:', 'moksa-for-woocommerce' ) . '</strong><span style="font-family:monospace;">' . esc_html( $track_no ) . '</span></p>';
		}
		if ( '' !== $status ) {
			echo '<p><strong>' . esc_html__( 'Shipping status:', 'moksa-for-woocommerce' ) . '</strong>' . esc_html( $status ) . '</p>';
		}
		...
		if ( '' === $smseid && '' === $store_id ) {
			echo '<p style="color:#646970;font-size:12px;margin-bottom:10px;">';
			echo esc_html__( 'No SmilePay shipment has been created yet.', 'moksa-for-woocommerce' );
			echo '</p>';
		}

		echo '<p style="margin-top:10px;padding-top:8px;border-top:1px dashed #c0c0c0;">';
		if ( '' === $pay_no && '' === $track_no ) {
			echo '<button type="button" class="button button-primary moksafowo-smilepay-shipping-create" data-order="' . esc_attr( (string) $order_id ) . '" data-nonce="' . esc_attr( $nonce_create ) . '">' . esc_html__( 'Create SmilePay shipment', 'moksa-for-woocommerce' ) . '</button>';
		} else {
			echo '<span style="color:#00a32a;">' . esc_html__( 'Shipment created', 'moksa-for-woocommerce' ) . '</span>';
		}
		echo '</p>';
