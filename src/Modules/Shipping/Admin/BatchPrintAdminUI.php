...
final class BatchPrintAdminUI {
	...
	public static function register_column( array $cols ): array {
		...
		foreach ( $cols as $k => $v ) {
			$new[ $k ] = $v;
			if ( 'order_status' === $k ) {
				$new['moksafowo_shipping_method']   = __( 'Shipping method', 'moksa-for-woocommerce' );
				$new['moksafowo_shipping_label_no'] = __( 'Shipping ID', 'moksa-for-woocommerce' );
			}
		}
	...
	public static function enqueue( string $hook ): void {
		...
		wp_localize_script(
			...
			[
				...
				'i18n'      => [
					'modal_title' => __( 'Bulk print', 'moksa-for-woocommerce' ),
					'order_no'    => __( 'Orders', 'moksa-for-woocommerce' ),
					'recipient'   => __( 'Recipient', 'moksa-for-woocommerce' ),
					'method'      => __( 'Shipping method', 'moksa-for-woocommerce' ),
					'status'      => __( 'Order status', 'moksa-for-woocommerce' ),
					'printable'   => __( 'Printable', 'moksa-for-woocommerce' ),
					'no_orders'   => __( 'There are no printable orders.', 'moksa-for-woocommerce' ),
					'select_one'  => __( 'Select at least one.', 'moksa-for-woocommerce' ),
					/* translators: %d: number of selected orders to print */
					'print'       => __( 'Print (%d)', 'moksa-for-woocommerce' ),
					'cancel'      => __( 'Cancel', 'moksa-for-woocommerce' ),
					'loading'     => __( 'Loading…', 'moksa-for-woocommerce' ),
					'error'       => __( 'Loading failed.', 'moksa-for-woocommerce' ),
					'yes'         => __( '✓', 'moksa-for-woocommerce' ),
					'no'          => __( '—', 'moksa-for-woocommerce' ),
					'paper_size'  => __( 'Paper:', 'moksa-for-woocommerce' ),
					'a4'          => __( 'A4 standard', 'moksa-for-woocommerce' ),
					'a6'          => __( 'A6 label printer', 'moksa-for-woocommerce' ),
				],
			]
		);
		...
		<div id="moksafowo-shipping-batch-print-modal" class="moksafowo-batch-modal" style="display:none;">
			<div class="moksafowo-batch-modal__panel">
				<div class="moksafowo-batch-modal__header">
					...
					<button type="button" class="moksafowo-batch-modal__close" aria-label="<?php esc_attr_e( 'Close', 'moksa-for-woocommerce' ); ?>">×</button>
				</div>
				...
				<div class="moksafowo-batch-modal__footer">
					<label class="moksafowo-batch-mode" style="margin-right:auto;display:flex;align-items:center;gap:8px;font-size:13px;">
						<span><?php esc_html_e( 'Paper:', 'moksa-for-woocommerce' ); ?></span>
						<select class="moksafowo-batch-mode__select">
							<option value="1"><?php esc_html_e( 'A4 standard', 'moksa-for-woocommerce' ); ?></option>
							<option value="2"><?php esc_html_e( 'A6 label printer', 'moksa-for-woocommerce' ); ?></option>
						</select>
					</label>
	...
	public static function ajax_list(): void {
		...
		if ( null === $provider ) {
			wp_send_json_error( [ 'message' => __( 'That shipping module could not be found.', 'moksa-for-woocommerce' ) ], 404 );
		}
	...
	public static function ajax_run(): void {
		...
		if ( empty( $ids ) ) {
			wp_send_json_error( [ 'message' => __( 'Select at least one order.', 'moksa-for-woocommerce' ) ], 400 );
		}

		$mode  = isset( $_POST['mode'] ) && '2' === sanitize_text_field( wp_unslash( $_POST['mode'] ) ) ? '2' : '1';
		$forms = self::run_provider( $provider, $ids, $mode );
		if ( empty( $forms ) ) {
			wp_send_json_error( [ 'message' => __( 'There is nothing to print.', 'moksa-for-woocommerce' ) ], 400 );
		}
	...
	public static function build_print_url( array $order_ids, string $mode = '1' ): array {
		...
		if ( empty( $ids ) ) {
			return [
				'ok'      => false,
				'message' => __( 'No order was specified.', 'moksa-for-woocommerce' ),
			];
		}
		$mode     = '2' === $mode ? '2' : '1';
		$registry = BatchPrintRegistry::all();
		...
		if ( empty( $forms ) ) {
			return [
				'ok'      => false,
				'skipped' => $skipped,
				'message' => __( 'The selected orders have no printable shipping labels — the shipments may not have been created yet.', 'moksa-for-woocommerce' ),
			];
		}
	...
	public static function register_bulk_actions( array $actions ): array {
		if ( ! empty( BatchPrintRegistry::all() ) ) {
			$actions[ self::BULK_ACTION ] = __( 'Print shipping labels (carrier detected automatically)', 'moksa-for-woocommerce' );
		}
		return $actions;
	}
	...
	public static function bulk_notices(): void {
		...
		printf(
			...
			esc_html__( 'None of the selected orders has a printable label for this carrier, so all of them were skipped.', 'moksa-for-woocommerce' )
		);
	}
	...
	public static function render_print_output(): void {
		...
		if ( '' === $token ) {
			wp_die( esc_html__( 'The print link has expired. Please try again.', 'moksa-for-woocommerce' ), 403 );
		}
		...
		if ( ! is_array( $forms ) || empty( $forms ) ) {
			wp_die( esc_html__( 'There is nothing to print, or the link has expired.', 'moksa-for-woocommerce' ) );
		}
		...
		<head>
			...
			<title><?php esc_html_e( 'Bulk print shipping labels', 'moksa-for-woocommerce' ); ?></title>
			<?php wp_print_styles( 'moksafowo-bp-print' ); ?>
		</head>
		<body>
			<h2><?php esc_html_e( 'Bulk print shipping labels', 'moksa-for-woocommerce' ); ?></h2>
			<?php if ( $skipped > 0 ) : ?>
				<?php /* translators: 1: number of labels produced, 2: number of skipped orders */ ?>
				<p><?php printf( esc_html__( '%1$d labels generated; %2$d orders skipped because they use another carrier.', 'moksa-for-woocommerce' ), (int) $count, (int) $skipped ); ?></p>
			<?php else : ?>
				<?php /* translators: %d: number of labels produced */ ?>
				<p><?php printf( esc_html__( '%d labels generated.', 'moksa-for-woocommerce' ), (int) $count ); ?></p>
			<?php endif; ?>

			<?php if ( ! $single ) : ?>
				<p><?php esc_html_e( 'Click the buttons below to open each label — this stops the browser from blocking pop-ups:', 'moksa-for-woocommerce' ); ?></p>
				<?php foreach ( $forms as $i => $spec ) : ?>
					<?php /* translators: %d: label sequence number */ ?>
					<button type="button" data-bp-target="moksafowo-bp-f<?php echo (int) $i; ?>"><?php printf( esc_html__( 'Print label %d', 'moksa-for-woocommerce' ), (int) $i + 1 ); ?></button>
				<?php endforeach; ?>
			<?php endif; ?>
