...
final class PaymentRequest {
	...
	public static function cancel( array $args ): array {
		return self::call_encrypted( self::PATH_CANCEL, array_merge( $args, [
			'RespondType' => 'JSON',
			'Version'     => '1.0',
	...
	public static function close( array $args ): array {
		return self::call_encrypted( self::PATH_CLOSE, array_merge( $args, [
			'RespondType' => 'JSON',
			'Version'     => '1.1',
			'TimeStamp'   => (string) time(),
			'CloseType'   => 1,
	...
	public static function query( string $merchant_order_no, int $amt ): array {
		...
		$resp = wp_remote_post( Helper::base_url() . self::PATH_QUERY, [
			'body'    => $args,
			'timeout' => 30,
		] );
		if ( is_wp_error( $resp ) ) {
		...
		if ( ! is_array( $decoded ) ) {
			return [ 'ok' => false, 'message' => __( '藍新查詢回傳格式錯誤', 'mo-ectools' ) ];
		}
		Helper::log( 'B02 query', [ 'mtn' => $merchant_order_no, 'status' => (string) ( $decoded['Status'] ?? '' ) ] );
		if ( 'SUCCESS' !== ( $decoded['Status'] ?? '' ) ) {
			return [ 'ok' => false, 'message' => sprintf( '%s', $decoded['Message'] ?? '' ) ];
		}
		return [ 'ok' => true, 'message' => 'OK', 'data' => (array) ( $decoded['Result'] ?? [] ) ];
	}
	...
	private static function call_wallet_style( string $path, array $args, string $version, bool $use_json = true ): array {
		...
		try {
			...
			return [ 'ok' => false, 'message' => 'AES-256-CBC encrypt failed' ];
		}
		$hash = strtoupper( hash( 'sha256', 'HashKey=' . Helper::hash_key() . '&' . $hex . '&HashIV=' . Helper::hash_iv() ) );

		$resp = wp_remote_post( Helper::base_url() . $path, [
			'body'    => [
		...
		// B06 SUCCESS = 1000；B07/B62 SUCCESS = "SUCCESS"
		$ok = ( '1000' === ( $decoded['Status'] ?? '' ) ) || ( 'SUCCESS' === ( $decoded['Status'] ?? '' ) );
		if ( ! $ok ) {
			return [ 'ok' => false, 'message' => (string) ( $decoded['Message'] ?? 'Unknown error' ) ];
		}
		// EncryptData 解密 (回包同樣加密)
		$inner = isset( $decoded['EncryptData'] ) ? Helper::decrypt_trade_info( (string) $decoded['EncryptData'] ) : null;
		return [ 'ok' => true, 'message' => (string) ( $decoded['Message'] ?? 'OK' ), 'data' => $inner ?? [] ];
	}
	...
	private static function call_encrypted( string $path, array $args ): array {
		...
		$body = wp_remote_retrieve_body( $resp );
		// Cancel 回傳是 query string，Close 回傳是 JSON （RespondType 影響）
		$decoded = json_decode( $body, true );
		if ( ! is_array( $decoded ) ) {
			parse_str( $body, $decoded );
		}
		...
		return [ 'ok' => true, 'message' => (string) ( $decoded['Message'] ?? 'OK' ), 'data' => $result ];
	}
